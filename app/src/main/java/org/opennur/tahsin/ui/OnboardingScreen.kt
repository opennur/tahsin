...
fun OnboardingScreen(
    ...
    Box(modifier = modifier.fillMaxSize().background(AyahColors.Background)) {
        Column(
            ...
            Spacer(modifier = Modifier.height(8.dp))
            GoalOption(
                label = strings.goalRecitation,
                selected = goal == LearningGoal.RECITATION,
                onClick = { goalKey = LearningGoal.RECITATION.key },
            )
            GoalOption(
                label = strings.goalUnderstanding,
                selected = goal == LearningGoal.UNDERSTANDING,
                onClick = { goalKey = LearningGoal.UNDERSTANDING.key },
            )
            GoalOption(
                label = strings.goalMemorization,
                selected = goal == LearningGoal.MEMORIZATION,
                onClick = { goalKey = LearningGoal.MEMORIZATION.key },
            )
            GoalOption(
                label = strings.goalArabic,
                selected = goal == LearningGoal.ARABIC,
                onClick = { goalKey = LearningGoal.ARABIC.key },
            )
            ...
            AyahCard(modifier = Modifier.fillMaxWidth()) {
                Column {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        AyahButton(
                            text = strings.languageNameId,
                            variant = if (language == AppLanguage.ID) {
                                AyahButtonVariant.Primary
                            } else {
                                AyahButtonVariant.Outline
                            },
                            modifier = Modifier.weight(1f),
                            onClick = { onSetLanguage(AppLanguage.ID) },
                        )
                        AyahButton(
                            text = strings.languageNameEn,
                            variant = if (language == AppLanguage.EN) {
                                AyahButtonVariant.Primary
                            } else {
                                AyahButtonVariant.Outline
                            },
